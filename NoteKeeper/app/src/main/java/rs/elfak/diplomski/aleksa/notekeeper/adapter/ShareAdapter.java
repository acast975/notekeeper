...
/**
 * Created by aleks on 25.9.2016..
 */

public class ShareAdapter extends RecyclerView.Adapter<FriendAdapterModel> {
    ...
    public ShareAdapter(Context context, List<User> users, int noteId) {
        userList = users;
        this.noteId = noteId;
    ...
    public void onBindViewHolder(final FriendAdapterModel holder, final int position) {
        ...
        holder.getDelete().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                builder.setMessage("Are you sure you want to stop sharing this note with " + holder.getusername().getText() + "?");

                builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {

                    public void onClick(DialogInterface dialog, int which) {
                        HttpConnector.stopSharingNote((new Session(context)).getId(), noteId, userList.get(holder.getAdapterPosition()).getId(), context, new CommonVolleyCallback() {
                            @Override
                            public void success() {
